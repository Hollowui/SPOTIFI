...
class CircularDoublyLinkedList:
    ...
    def append(self, value):
        self._dll.append(value)
        if self.current_node is None:
            self.current_node = self._dll.first
    ...
    def move_next(self):
        ...
        if self.current_node is None:
            ...
            self.current_node = self.current_node.next or self._dll.first
    ...
    def move_prev(self):
        if self.is_empty():
            ...
            return None
        if self.current_node is None:
            self.current_node = self._dll.last
        else:
            self.current_node = self.current_node.prev or self._dll.last
        return self.current_node.value
    ...
    def remove(self, value):
        ...
        while node is not None:
            if node.value == value:
                ...
                if self.current_node is None and len(self._dll) > 0:
                    self.current_node = self._dll.first
                return True
            node = node.next
        return False
    ...
    @property
    def first(self):
        return self._dll.first

    @property
    def last(self):
        return self._dll.last


class MusicPlayerPlaylist(CircularDoublyLinkedList):
    def __init__(self):
        super().__init__()

    def add_song(self, song):
        if song.playlist_id is None:
            song.playlist_id = len(self) + 1
        self.append(song)
    ...
    def load_playlist(self, filename, Song):
        import json
...
def load_album_gui():
    ...
    playlist.current_node = playlist.first
    update_playlist_display()
...
def load_playlist_gui():
    ...
    if filename:
        playlist.load_playlist(filename, Song)
        update_playlist_display()
        if playlist.current_node:
            current_song_label.config(text=f"{playlist.current_song()}")
